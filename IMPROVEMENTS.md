...
### P4-A — 상수는 그대로 두고, 근거 도구를 먼저 넣었다

기록이 **3판**(전부 `_test-turtle-soup`, 그중 1판은 1질문 S랭크 테스트)뿐이라
관측치로 상수를 고칠 근거가 없다. AC가 요구하는 "변경마다 수치 근거"를 지키려면
표본이 먼저다. 그래서 이번에는 값을 바꾸지 않고 다음을 넣었다:

- `src/lib/balance.ts` + `scripts/balance-report.ts` (`npm run balance`) — 사건별
  클리어율·클리어 시 평균 남은 토큰·질문 수·S 비율과, 기록이 없어도 나오는
  "현재 상수의 도달 가능성"을 출력한다. 사건당 30판(`MIN_SAMPLE`) 미만이면 "부족"으로 표시.
- `src/lib/gameConfig.ts`에 각 상수의 의미와 조정 레버를 주석으로 기록.

현재 상수로 계산되는 도달 가능성(정확도 100% / 70%, 힌트·미리보기 미구매):

| 랭크 | 임계 | 정확도 100% | 정확도 70% |
|---|---|---|---|
| S | 850 | 9질문 | 5질문 |
| A | 700 | 18질문 | 15질문 |
| B | 500 | 31질문 | 27질문 |
...
점수 비중은 토큰 80% / 정확도 20%다(힌트 -80점, 미리보기 -160점, 오답 -80점).
"적게 묻는 것"이 "정확히 맞히는 것"보다 4배 세게 반영된다는 뜻이라, 실제 플레이가
쌓이면 이 비중부터 재볼 것. 표본이 모이면 `npm run balance` 출력을 근거로 조정한다.

### P4-B — 데일리 챌린지 / 리더보드

- **선택은 날짜만으로**: `src/lib/daily.ts`가 KST 날짜 키(`YYYY-MM-DD`)를 FNV-1a로
  해싱해 정렬된 후보 중 하나를 고른다. 저장소가 없으니 서버가 몇 대든, 파일 DB든
  Supabase든 같은 날 같은 사건이 나온다. 후보 목록(발행 사건)이 바뀌면 그날의 선택이
  바뀔 수 있어서, 리더보드는 **(날짜, 사건 id)** 두 개를 키로 쓴다 — 기록이 섞이지 않는다.
  후보에서 `_`로 시작하는 테스트 사건은 항상 뺀다.
- **기록은 서버가**: 점수를 받는 POST는 만들지 않았다. `/api/verdict`가 채점을 끝내며
  `submitResult()`를 직접 호출하고, `/api/leaderboard`는 조회 전용이다.
- **조작 방어 3겹**: ① 클라이언트가 점수를 보낼 통로 자체가 없다 ② `plausibleTokens()`가
  "질문 수·오답 수로 가능한 최대 토큰"을 넘는 값을 잘라낸다(힌트·미리보기는 토큰을 더
  깎기만 하므로 상한으로 안전) ③ 점수는 잘린 토큰과 서버 계산 정확도로 다시 구한다.
  더해서 `RATE_LIMIT_VERDICTS_PER_MINUTE=6`, (날짜·사건·IP해시)당 최고 기록 1개만 유지.
  실측: `tokens: 999, questions: 12`로 제출해도 38로 잘려 808점이 되고, 기존 기록보다
  높지 않으면 갱신되지 않았다. **남는 위험**은 질문 수 자체를 줄여 보내는 조작 —
  막으려면 서버가 판마다 세션을 들어야 해서 범위 밖으로 두고 코드에 명시했다.
- IP는 해시(`ADMIN_SESSION_SECRET` 솔트)로만 저장한다. 스키마는
  `supabase/schema.sql`의 `daily_leaderboard`, 파일 DB는 `data/leaderboard.json`.
- 화면: `/daily`(오늘의 사건 카드 + 닉네임 + Top 20 + KST 자정 카운트다운),
  홈 메뉴 "오늘의 사건", 결과 화면의 순위 배너.

### P4-C — 사운드 / 햅틱

- **음원 파일을 두지 않았다.** `src/lib/sound.ts`가 WebAudio로 직접 합성한다.
  100~600ms짜리 판정음에 바이너리 에셋과 라이선스를 끌어올 이유가 없고, 네트워크
  왕복이 없어 판정이 뜨는 순간과 소리가 어긋나지 않는다. (지시서의 `public/sfx/`
  항목에서 의도적으로 벗어난 유일한 지점이다.)
- 이벤트: 판정 YES/NO/기타, 삽화 해금, 클리어, 게임오버, 오답, 토큰 경고선(10Q)
  진입. 진동은 `navigator.vibrate` 지원 시에만.
- 자동재생 정책: `AudioContext`는 첫 재생 요청 때 만들고, 사용자 조작(질문 전송·
  최종 추리)에서 `primeAudio()`로 깨운다. 실패는 조용히 삼켜 콘솔을 더럽히지 않는다.
- 설정 연결: `/settings`의 효과음·진동 토글이 `yesno_settings`를 통해 실제로 재생을
  좌우한다. 켠 순간 미리듣기가 나가도록 저장을 setState 밖에서 동기로 처리했다.

**남겨둔 것**: 멀티플레이 방(`/room/[code]`) 화면에는 아직 소리를 붙이지 않았다.
단일 플레이와 판정 도착 경로가 달라 별도 작업이 필요하다.
